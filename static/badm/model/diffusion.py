...
a='./data/test/motions_sliced/gJS_sBM_cAll_d03_mJS3_ch02_slice6.pkl'
with open(a, 'rb') as f:
    pose_data = pickle.load(f)

# print(pose_data['pos'].shape) 300 3
# print(pose_data['q'].shape) 300 72
...
class GaussianDiffusion(nn.Module):
    ...
    def inpaint_loop(
        ...
        mask[:, :, 7:] *= (
            torch.tensor(down_joint_mask, device=x.device).repeat_interleave(6).unsqueeze(0).unsqueeze(0))



        # value = constraint["value"].to(device)  # batch x horizon x channels

        value = pose_data.to(device)
        start_point = self.n_timestep if start_point is None else start_point
        for i in tqdm(reversed(range(0, start_point))):
            # fill with i
            timesteps = torch.full((batch_size,), i, device=device, dtype=torch.long)
  ...
  #      p_map(inner, enumerate(poses))
        ...
        if fk_out is not None and mode != "long":
            Path(fk_out).mkdir(parents=True, exist_ok=True)
            for num, (qq, pos_, filename, pose) in enumerate(tqdm(zip(q, pos, name, poses))):
