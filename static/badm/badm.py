...
from model.S0930d import DanceDecoder
...
class badm:
    def __init__(
        ...
        feature_dim = 4800 # if use_baseline_feats else 4800
    ...
    def train_loop(self, opt):
        ...
        for epoch in range(1, opt.epochs + 1):
            ...
            # train
            self.train()
            for step, (x, cond, beat, filename, wavnames) in enumerate(
                load_loop(train_data_loader)
            ):
              #  cond = torch.cat([cond, beat], dim=-1)
               # cond = beat
            ...
            if (epoch % opt.save_interval) == 0:
                ...
                if self.accelerator.is_main_process:
                    ...
                    #(x, cond, beat, filename, wavnames) = next(iter(test_data_loader))
                    for step, (x, cond, beat,  filename, wavnames) in enumerate(load_loop(test_data_loader)):
                       # print(x.size())
                       # cond = torch.cat([cond, beat], dim=-1)
                      #  cond = beat
                        ...
                        self.diffusion.render_sample(
                                ...
                                sound=True,
                                fk_out = os.path.join(opt.render_dir, "train_" + opt.exp_name, 'testout')
                               )
    ...
    def render_sample(
        ...
        # print(cond.shape)
        print(wavname[:render_count], 'wav_name')
