...
#        print(self.data["filenames"][idx])

        filename_, beat_name = self.data["filenames"][idx]
        feature = torch.from_numpy(np.load(filename_))
        beat = torch.from_numpy(np.load(beat_name))
        return self.data["pose"][idx], feature, beat, filename_, self.data["wavs"][idx]
    ...
    def load_aistpp(self):
        ...
        beat_path = os.path.join(split_data_path, "baseline_feats")
        ...
        for motion, feature, beat, wav in zip(motions, features, beats,  wavs):
