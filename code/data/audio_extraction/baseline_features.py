...
def extract(fpath, skip_completed=True, dest_dir="aist_baseline_feats"):
    ...
    data, d2 = librosa.load(fpath, sr=SR)
    ...
    # print(beat_onehot.size())

    audio_feature = np.concatenate(
        [envelope[:, None], mfcc, chroma, peak_onehot[:, None], beat_onehot[:, None]],
        axis=-1,
    )
    print(beat_onehot.shape)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--src", help="source path to AIST++ audio files", default='custom_music')
    parser.add_argument("--dest", help="dest path to audio features", default='feat')

    args = parser.parse_args()

    extract_folder('./custon_music', 'feature')
